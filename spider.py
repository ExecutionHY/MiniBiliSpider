...
def main():
	...
	for user_id in range(MIN_USER_ID, MAX_USER_ID):
		...
		pd.DataFrame([[tag['tag'], tag['created'], tag['typeid']] for tag in info['tags']]).to_csv('out/video_tag.csv', mode='a', encoding='utf-8', header=None)
